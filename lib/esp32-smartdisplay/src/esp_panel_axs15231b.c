...
esp_err_t esp_lcd_new_panel_axs15231b(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    *ret_panel = &(axs15231b->base);
    ...
    ESP_LOGI(TAG, "LCD panel create success, version: %d.%d.%d", ESP_LCD_AXS15231B_VER_MAJOR, ESP_LCD_AXS15231B_VER_MINOR, ESP_LCD_AXS15231B_VER_PATCH);
...
static esp_err_t panel_axs15231b_init(esp_lcd_panel_t *panel)
{
    printf("JOS - panel_axs15231b_init()\n");
    ...
    // JOS : testing
    // tx_param(axs15231b, io, LCD_CMD_MADCTL, (uint8_t[]) { 1 << 6 }, 1);
    // tx_param(axs15231b, io, 0x2F, NULL, 0); // JOS : send 0x2F
    // printf("JOS : MADCTL = %02X\n", axs15231b->madctl_val);

    tx_param(axs15231b, io, 0x2F, (uint8_t[]){0xFF}, 1);

    const int n = 2;
    uint8_t buff[n];

    esp_lcd_panel_io_rx_param(io, 0x2F, buff, n);
    for (int i = 0; i < n; i++)
    {
        printf("JOS rx[%d]=0x%02X\n", i, buff[i]);
    }

    printf("JOS - panel_axs15231b_init() done\n");

    return ESP_OK;
}

extern uint16_t flush_cnt;
...
static esp_err_t panel_axs15231b_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    // JOS : TEST, set partial mode on
    // tx_param(axs15231b, io, LCD_CMD_PTLON, NULL, 0);
...
//    {
    printf("RASET : y->y: %03d->%03d \n", y_start, y_end-1);
    ESP_RETURN_ON_ERROR(tx_param(axs15231b, io, LCD_CMD_RASET, (uint8_t[]){
        (y_start >> 8) & 0x0F,
        y_start & 0xFF,
        ((y_end - 1) >> 8) & 0x0F,
        (y_end - 1) & 0xFF,
    }, 4), TAG, "RASET send command failed");

    printf("CASET : x->x: %03d->%03d \n", x_start, x_end-1);
    ...
    // const uint8_t caset[4] = {(x_start >> 8) & 0xF, x_start & 0xFF, ((x_end - 1) >> 8) & 0xF, (x_end - 1) & 0xFF};
    // const uint8_t raset[4] = {(y_start >> 8) & 0xF, y_start & 0xFF, ((y_end - 1) >> 8) & 0xF, (y_end - 1) & 0xFF};
    ...
    //  tx_color(axs15231b, io, LCD_CMD_TEOFF, NULL, 0); // TEAR OFF

    // tx_color(axs15231b, io, LCD_CMD_PTLON, NULL, 0); // PARTIAL MODE ON
    // tx_color(axs15231b, io, LCD_CMD_PTLAR, (uint8_t[]){0, 0, (480>>8), 480 & 0xFF}, 4); // PARTIAL MODE ON
    // tx_color(axs15231b, io, 0x31, (uint8_t[]){0, 10, (300>>8), 300 & 0xFF}, 4); // PARTIAL MODE ON
    ...
    if (flush_cnt == 0)
    {
        // start at given address
        printf("RAMWR : %5d, x1=%03d, y1=%03d, x2=%03d, y2=%03d, len=%06d, flush_cnt=%d\n", cnt++, x_start, y_start, x_end, y_end, len, flush_cnt);
        tx_color(axs15231b, io, LCD_CMD_RAMWR, color_data, len);
    }
    else
    {
        printf("RAMWRC : %5d, x1=%03d, y1=%03d, x2=%03d, y2=%03d, len=%06d, flush_cnt=%d\n", cnt++, x_start, y_start, x_end, y_end, len, flush_cnt);
        // continue
